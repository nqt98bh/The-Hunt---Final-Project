...
public class FrozenFire : ISkill
{
    float coolDown = 2f;
    ...
    public bool CanUse(BossAI boss, CharacterController player, float attackRange)
    {
        ...
        if ((boss.GetCurrentHP() >= (bossConfig.maxHealth/3) && boss.GetCurrentHP() < (bossConfig.maxHealth *2/3)) && distance < attackRange && Time.time - lastTimeUse > coolDown ) 
        {
